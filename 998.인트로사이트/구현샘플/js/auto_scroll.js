...
// 1.사용할 이벤트 : wheel
//  ㄴ> scroll 이벤트는 스크롤바가 움직일 때 발생한다.
//   ㄴ> wheel 이벤트는 마우스 휠이 움직일 때 발생한다.

// 2.이벤트 대상 : window

// 3.body에 overflow:hidden 셋팅
document.body.style.overflow = "hidden";
// 4.html에 스크롤 부드럽게 움직이게 설정하기
document.documentElement.style.scrollBehavior = "smooth";
//  html최상위 요소는 document.documentElement로 선택한다

// 5.새로고침시 스크롤 위치 맨위로 이동하기
...
// 7.이동 단위설정 -> window의 높이값
let winH = window.innerHeight;

// 8.전체.page인 페이지 개수 구하기
//  ㄴ> 페이지 증가 한계값으로 사용하기!
const pg = document.querySelectorAll(".page");
const pgcnt = pg.length;
console.log("페이지개수:", pgcnt);
...
// 그래야 한 영역씩 움직이며 자동 스크롤이동을 구현 할 수 있다!
...
window.addEventListener(
  ...
  (e) => {
    ...
    // (1)기본기능 막기
    e.preventDefault();
    // ㄴ> window/ document/ body 에서 기본기능 막기를 할 때
    // passive 모드값을 false로 세팅해야 에러가 없이 작동한다!!!!
    // addEventListener(이벤트명,함수,{passive:false})<< 이 형식을 window/ document/ body에서 사용해야함
    ...
    if (dir > 0) {
      ...
      //아래방향 음수
      pgNo++; //페이지 증가
      if (pgNo >= pgcnt) pgNo = pgcnt - 1; // 한계값 고정시키기
    }
...
function touchEndFn(e) {
  ...
  if (pgNo < 0) pgNo = 0;
  else if (pgNo > pgcnt - 1) pgNo = pgcnt - 1;
